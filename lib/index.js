...
async function buildInstallEnv() {
  ...
  return { env, proxy, detected }
}

/** 仅补缺失键，不覆盖用户已有 .npmrc 设置。 */
...
function isNetworkTimeoutLog(log) {
  return /TimeoutError|operation was aborted due to timeout|timed?\s*out|error\s*\(23\)|codeload\.github\.com|ENOTFOUND|ECONNRESET|ECONNREFUSED|EAI_AGAIN|network\s*socket|fetch failed/i.test(String(log || ''))
}
...
function githubRepoFromSpec(spec) {
  const match = /^github:([A-Za-z0-9_.-]+\/[A-Za-z0-9_.-]+)/.exec(String(spec || ''))
  return match ? match[1] : ''
}

/** 软探测：不通只警告，不阻断安装（避免误杀可达但 HEAD 受限的环境）。 */
async function probeGithubReachable(spec) {
  ...
  try {
    ...
    try {
      ...
      return { ok: false, note: `GitHub 探测返回 HTTP ${res.status}，安装可能失败。` }
    } finally {
      clearTimeout(timer)
    }
  } catch (error) {
    const msg = error instanceof Error ? error.message : String(error)
    return { ok: false, note: `无法连通 GitHub（${msg}）。若未配置代理，安装很可能超时。` }
  }
}

function plainDependencyHint(spec) {
  const installed = installedBundles()
  const specs = specMap()
  const hit = matchInstalled({ spec, id: spec.replace(/^github:/, '') }, installed, specs)
  if (!hit) return ''
  if (hit.bundled) return ''
  return '已安装为普通依赖（未声明 dsh.bundle），不会进入 profile bundles；重启后通常不会作为插件层加载。'
}
...
async function pumpInstallQueue() {
  ...
  ensureProfileNpmrc()
  const { env, proxy, detected } = await buildInstallEnv()
  if (next.cancelled) {
  ...
  const probe = await probeGithubReachable(next.spec)
  if (next.cancelled) {
    installJobMap.delete(next.id)
    installPumpBusy = false
    void pumpInstallQueue()
  ...
  if (next.cancelled) {
    ...
    const hint = plainDependencyHint(next.spec)
    next.status = 'success'
    next.hint = hint
    next.plainDependency = Boolean(hint)
        ...
        if (req.method === 'POST' && (path === '/install' || path === '/update' || path === '/repair' || path === '/uninstall')) {
          ...
          if (path === '/install') {
            ...
            if (result.ok) {
              ...
              const hint = plainDependencyHint(spec)
              return send(200, {
                ...result,
                needRestart: true,
                plainDependency: Boolean(hint),
                hint,
                log: (result.log || '') + (hint ? `\n${hint}` : ''),
              })
            }
          ...
          if (path === '/repair') {
            ...
            if (result.ok) {
              ...
              const hint = plainDependencyHint(target)
              return send(200, {
                ...result,
                needRestart: true,
                plainDependency: Boolean(hint),
