...
async function fetchCatalog(query, fresh) {
  ...
  if (!fresh && cached && Date.now() - cached.at < CACHE_MS) return filterItems(cached.items)

  const url = 'https://api.github.com/search/repositories?q=topic:dsh-plugin&sort=stars&order=desc&per_page=50'
  const res = await fetch(url, { headers: githubHeaders() })
  if (!res.ok) {
    const text = await res.text()
    if (cached?.items) return filterItems(cached.items)
    throw new Error(`GitHub 索引失败 ${res.status}: ${text.slice(0, 200)}`)
  }
  const body = await res.json()
  const items = (body.items || [])
    .filter((repo) => !EXCLUDE.has(repo.full_name))
    .map((repo) => ({
      id: repo.full_name,
      name: repo.name,
  ...
  writeJson(cachePath, { at: Date.now(), items })
    ...
    handler: async (req, res) => {
      ...
      try {
        ...
        if (req.method === 'GET' && path === '/catalog') {
          const items = await fetchCatalog(url.searchParams.get('q') || '', url.searchParams.get('fresh') === '1')
          const installed = installedBundles()
          return send(200, {
            ok: true,
