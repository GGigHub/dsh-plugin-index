...
function plainDependencyGuide(spec, { installed = false } = {}) {
  ...
  if (override?.path) {
    ...
    const steps = installed
      ? [
        ...
        `dsh plugin --profile web add ./${dir}/${sub}`,
        `# 3) 重启 DSH`,
      ]
      : [
        `# 不要点市场「安装」根仓；直接装子目录：`,
  ...
  return {
    ...
    installCommand: [
      ...
      `git clone https://github.com/${repo}`,
      `dsh plugin --profile web add ./${dir}/<子目录>`,
      `# 然后重启 DSH`,
    ].join('\n'),
  }
}
...
function runPlugin(args) {
  ...
  return new Promise((resolve) => {
    ...
    let out = ''
    child.stdout?.on('data', (chunk) => { out = appendLog(out, chunk) })
    child.stderr?.on('data', (chunk) => { out = appendLog(out, chunk) })
      ...
      try {
        ...
        if (req.method === 'POST' && (path === '/install' || path === '/update' || path === '/repair' || path === '/uninstall')) {
          ...
          if (path === '/install') {
            ...
            const plain = result.ok ? plainDependencyPayload(spec) : { plainDependency: false }
            const hint = plain.hint ? `\n${plain.hint}` : ''
            const cmd = plain.installCommand ? `\n\n手动安装命令：\n${plain.installCommand}` : ''
            return send(200, {
              ...result,
              needRestart: result.ok,
              ...plain,
          ...
          if (path === '/repair') {
            ...
            const plain = result.ok ? plainDependencyPayload(target) : { plainDependency: false }
            const hint = plain.hint ? `\n${plain.hint}` : ''
            const cmd = plain.installCommand ? `\n\n手动安装命令：\n${plain.installCommand}` : ''
            return send(200, {
              ...result,
