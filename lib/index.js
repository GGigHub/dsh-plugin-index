...
function mapRepos(repos) {
  return (repos || [])
    .filter((repo) => repo?.full_name && !EXCLUDE.has(repo.full_name) && !repo.archived)
    .map((repo) => ({
      id: repo.full_name,
      name: repo.name,
...
async function fetchCatalogPage(page) {
  const q = encodeURIComponent('topic:dsh-plugin archived:false')
  const url = `https://api.github.com/search/repositories?q=${q}&sort=stars&order=desc&per_page=${CATALOG_PER_PAGE}&page=${page}`
...
function cacheUsable(cached) {
  return Boolean(
    ...
    && (!cached.items.length || cached.items.some((item) => item && item.pushedAt)),
  )
}
...
/** pnpm 供应链拦截信号：需要放行构建脚本（allowBuilds）才能继续安装。 */
const ALLOW_BUILDS_SIGNALS = [
...
/** 从 pnpm 失败日志里提取需要放行的包 key（不带 `: true` 后缀）。 */
function allowBuildsKeysFromLog(log) {
  const text = String(log || '')
  if (!ALLOW_BUILDS_SIGNALS.some((signal) => text.includes(signal))) return new Set()
  const keys = new Set()
  // pnpm 会打印示例段："allowBuilds:\n  <key>: true"（git+ 与 codeload 两种形式都收）
  const example = text.match(/allowBuilds:\s*\r?\n((?:\s*[^\r\n]+:\s*true\r?\n?)+)/)
  if (example) {
    for (const line of example[1].split(/\r?\n/)) {
      const entry = line.trim()
      if (!entry) continue
      const key = entry.replace(/:\s*true\s*$/, '').trim()
      if (key) keys.add(key)
    }
  }
  // IGNORED_BUILDS："Ignored build scripts: node-pty@1.1.0, foo@2.0.0"
  const ignored = text.match(/Ignored build scripts:\s*([^\r\n]+)/)
  if (ignored) {
    for (const part of ignored[1].split(',')) {
      const name = String(part).trim().replace(/@[^@]+$/, '')
...
/** 把 key 合并进 profile 的 pnpm-workspace.yaml 的 allowBuilds 段；写失败软返回 error。 */
function applyAllowBuilds(keys) {
  const file = join(profileDir(), 'pnpm-workspace.yaml')
  const report = { added: [], changed: [], error: '' }
  try {
    let text = ''
    try {
    ...
    // 1) 把 pnpm 自动写入的占位符 "key: set this to true or false" 改成 true
    const placeholder = /^(\s*)(.+?)(\s*:\s*)set this to true or false\s*$/
    const normalized = []
    for (const line of lines) {
      const m = placeholder.exec(line)
      if (m) {
        normalized.push(`${m[1]}${m[2].trim()}${m[3]}true`)
        report.changed.push(m[2].trim())
      } else {
        normalized.push(line)
      }
    }
    // 2) 定位 allowBuilds 段（map 形式）的范围
    let allowStart = -1
    let allowEnd = -1
    for (let i = 0; i < normalized.length; i++) {
      if (/^allowBuilds:\s*$/.test(normalized[i].trim())) {
        allowStart = i
        allowEnd = i
        let j = i + 1
        while (j < normalized.length) {
          const t = normalized[j] || ''
          if (!t.trim() || /^\s/.test(t) || /^#/.test(t.trim())) {
            allowEnd = j
            j++
          } else {
            break
    ...
    if (allowStart >= 0) {
      for (let i = allowStart + 1; i <= allowEnd; i++) {
        const m = /^(\s+)(.+?):\s*(?:true|false)\s*$/.exec(normalized[i] || '')
        if (m) existing.add(m[2].trim())
      }
    }
    // 4) 追加缺失的 key
    ...
    if (toAdd.length) {
      if (allowStart >= 0) {
        normalized.splice(allowEnd + 1, 0, ...toAdd.map((key) => `  ${key}: true`))
      } else {
        if (normalized.length && normalized[normalized.length - 1].trim() !== '') normalized.push('')
        normalized.push('allowBuilds:')
        normalized.push(...toAdd.map((key) => `  ${key}: true`))
      }
      report.added.push(...toAdd)
    }
    const next = normalized.join(nl).replace(/\n{3,}/g, '\n\n').trimEnd() + '\n'
...
function formatInstallFailure(log, { proxy } = {}) {
  ...
  const tips = [
    ...
    '本次是网络问题，不是 allowBuilds / 构建脚本拦截；构建脚本拦截会由插件自动放行并重试。',
    '',
    '原始日志：',
    raw,
  ]
...
function catalogSnapshotItems() {
  if (catalogJob?.items?.length) return catalogJob.items
  const cached = readCatalogCache()
  return Array.isArray(cached?.items) ? cached.items : []
}

function catalogHitForSpec(spec) {
  const key = String(spec || '')
  if (!key) return null
  return catalogSnapshotItems().find((item) => item.spec === key) || null
}
...
function enqueueInstallJob(spec, title, extra = {}) {
  const id = String(++installJobSeq)
  const job = {
    id,
    spec,
    title: title || spec.replace(/^github:/, ''),
    description: String(extra.description || catalogHitForSpec(spec)?.description || '').trim(),
    status: 'installing',
    log: '排队中…',
    error: '',
    startedAt: null,
...
async function pumpInstallQueue() {
  ...
  try {
    ...
    // 网络重试耗尽后仍失败：若命中 pnpm 构建脚本拦截（allowBuilds），自动放行并重试一次。
    // 与网络重试区分开：allowBuilds 修复是确定性的，不占用 INSTALL_NETWORK_RETRIES。
    if (!result.ok && !next.cancelled) {
      const keys = allowBuildsKeysFromLog(result.log)
      if (keys.size) {
        const fixed = applyAllowBuilds(keys)
        const fixedNames = [...fixed.added, ...fixed.changed]
        if (fixedNames.length) {
          next.log = appendLog(next.log, `\n检测到 pnpm 构建脚本拦截，已自动放行：${fixedNames.join('、')}\n正在重试…\n`)
          const { promise, kill } = spawnPlugin(['add', next.spec], (chunk) => {
            if (next.cancelled) return
            next.log = appendLog(next.log, chunk)
          }, env)
          ...
          if (result.ok) {
            ...
            next.log = appendLog(next.log, fixed.error ? `\n自动放行失败：${fixed.error}\n` : '\n自动放行后仍失败，请查看上方日志。\n')
          }
        } else if (fixed.error) {
          next.log = appendLog(next.log, `\n检测到构建脚本拦截，但自动放行失败：${fixed.error}\n`)
        }
      }
    }
...
function retryInstallJob(id) {
  ...
  installJobMap.delete(id)
  const next = enqueueInstallJob(spec, title, { description })
    ...
    handler: async (req, res) => {
      ...
      try {
        ...
        if (req.method === 'POST' && path === '/jobs/install') {
          ...
          const job = enqueueInstallJob(spec, title || spec.replace(/^github:/, ''), { description })
