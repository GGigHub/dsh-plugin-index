window.__ModuleLoader__.load({
  ...
  factory: (require) => {
    ...
    function Overlay(props) {
      ...
      var sourceKey = (source[0] && (source[0].id || source[0].name)) || ''
        ...
        createElement('div', { style: overlay.panel },
          ...
          createElement('div', { style: overlay.toolbar },
            createElement('input', {
              style: overlay.input,
              value: query,
              placeholder: '搜索仓库名或描述（本地过滤）',
              onChange: function (e) { setQuery(e.target.value) },
              onKeyDown: function (e) { if (e.key === 'Enter') onSearch() },
            }),
            createElement('button', { type: 'button', style: overlay.btn, onClick: onSearch, disabled: loading && !catalog.length }, loading && !catalog.length ? '刷新中…' : (partial ? '补全中…' : '刷新')),
          ...
          !(loading && source.length === 0) && rows.length > 0 && createElement('div', { style: overlay.pager },
            createElement('button', {
    ...
    function FooterButton(props) {
      ...
      var reqSeq = useState({ catalog: 0, installed: 0 })[0]
