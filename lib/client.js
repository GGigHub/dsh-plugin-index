window.__ModuleLoader__.load({
  ...
  factory: (require) => {
    ...
    var RISK_WARNING = '本列表为社区仓库索引，所有插件均未检查木马、病毒或恶意代码。请自行 clone Git 仓库、审查源码后再决定是否安装。'
    var PAGE_SIZE = 50
    var CATEGORIES = [
      { id: 'all', label: '全部' },
      { id: 'ui', label: '界面' },
    ...
    var ACTIVITY = [
      { id: '1', label: '本周', weeks: 1 },
      { id: '2', label: '2 周内', weeks: 2 },
      { id: '4', label: '4 周内', weeks: 4 },
      { id: '8', label: '8 周内', weeks: 8 },
      { id: 'all', label: '含停滞', weeks: null },
    ]

    function weeksSincePush(item, now) {
      if (item && item.archived) return Infinity
      var t = Date.parse((item && item.pushedAt) || '')
      if (!t) return null
      return Math.floor(Math.max(0, (now - t) / WEEK_MS))
    }

    function matchesActivity(item, window, now) {
      if (!window || window.weeks == null) return true
      var weeks = weeksSincePush(item, now)
      if (weeks == null) return true
      return weeks < window.weeks
    }
    ...
    function fmtWeeksAgo(weeks) {
      if (weeks == null || weeks === Infinity) return ''
      if (weeks === 0) return '本周更新'
      return weeks + ' 周前'
    }

    function fmtPushedAt(pushedAt) {
      var t = Date.parse(pushedAt || '')
      if (!t) return ''
      var d = new Date(t)
    ...
    function fmtActivityMeta(item, weeks) {
      var weekLabel = fmtWeeksAgo(weeks)
      var dateLabel = fmtPushedAt(item && item.pushedAt)
      if (weekLabel && dateLabel) return weekLabel + ' · ' + dateLabel
      return weekLabel || dateLabel
    }

    function activityRisk(weeks) {
      if (weeks == null) return null
      if (weeks === Infinity) {
        return { label: '已归档', style: 'tagDanger', title: '仓库已归档，通常不再维护' }
      }
      if (weeks >= 4) {
        return {
          label: '可能停更',
          style: 'tagWarn',
          title: '按 git 最后一次 push 已超过 4 周。DSH 官方几乎一周更新多次，此插件可能不兼容当前版本。',
        }
      }
      if (weeks >= 1) {
        return {
          label: '可能落后',
          style: 'tagWarn',
          title: '按 git 最后一次 push 已超过 1 周。DSH 官方几乎一周更新多次，此插件可能尚未跟上。',
        }
      }
      return null
    }
    ...
    function catalogHit(item, lookup) {
      if (!item || !lookup) return null
      return lookup[item.spec] || lookup[item.id] || lookup[item.name] || lookup[item.packageName] || null
    }

    function enrichFromCatalog(item, lookup) {
      if (!item) return item
      var hit = catalogHit(item, lookup)
      var extra = {}
      if (hit) {
        extra.categories = itemCategories(hit)
        extra.categoryLabels = itemCategoryLabels(hit)
        extra.category = hit.category
        extra.categoryLabel = hit.categoryLabel
        extra.description = item.description || hit.description || ''
        if (item.stars == null && hit.stars != null) extra.stars = hit.stars
        if (item.forks == null && hit.forks != null) extra.forks = hit.forks
        if (!item.language && hit.language) extra.language = hit.language
        if (!item.url && hit.url) extra.url = hit.url
        if (!item.pushedAt && hit.pushedAt) extra.pushedAt = hit.pushedAt
        if (item.archived == null && hit.archived != null) extra.archived = hit.archived
      } else if (!(Array.isArray(item.categories) && item.categories.length)) {
        extra.categories = ['other']
        extra.categoryLabels = ['其他']
        extra.category = 'other'
        extra.categoryLabel = '其他'
      }
    ...
    function Overlay(props) {
      ...
      var activityState = useState('4')
      var activity = activityState[0]
      var setActivity = activityState[1]
      ...
      var showActivity = tab === 'catalog'
      var activeActivity = ACTIVITY.find(function (w) { return w.id === activity }) || ACTIVITY[2]
      var catLookup = {}
      ...
      var source = tab === 'installed'
        ? installed.map(function (item) { return enrichFromCatalog(item, catLookup) })
        : tab === 'jobs'
          ? jobs.map(function (item) { return enrichFromCatalog(item, catLookup) })
          : catalog
      ...
      ACTIVITY.forEach(function (w) {
        activityCounts[w.id] = showActivity
          ? searched.filter(function (item) { return matchesActivity(item, w, now) }).length
          : 0
      })
      var afterActivity = showActivity
        ? searched.filter(function (item) { return matchesActivity(item, activeActivity, now) })
        : searched
      var categoryCounts = {}
      CATEGORIES.forEach(function (c) { categoryCounts[c.id] = 0 })
      afterActivity.forEach(function (item) {
        itemCategories(item).forEach(function (id) {
          categoryCounts[id] = (categoryCounts[id] || 0) + 1
        })
      })
      categoryCounts.all = afterActivity.length
      var rows = !showCategories || category === 'all'
        ? afterActivity
        : afterActivity.filter(function (item) { return itemCategories(item).indexOf(category) >= 0 })
      ...
      useEffect(function () {
        setPage(1)
      }, [tab, qn, source.length, sourceKey, category, activity])

      var subText = isJobs
        ? ('安装队列 · ' + jobs.length + ' 项' + (jobsActive ? ' · ' + jobsActive + ' 进行中' : '') + (jobsFailed ? ' · ' + jobsFailed + ' 失败' : '') + (jobsSuccess ? ' · ' + jobsSuccess + ' 成功' : ''))
        : (
          '按 GitHub 星标排序 · ' + (tab === 'catalog' ? '默认近 4 周有 git 活动 · ' : '') + 'topic: dsh-plugin · 已索引 ' + (tab === 'installed' ? installed.length : catalog.length) + ' 个' +
          (tab === 'catalog' && githubTotal ? ' / GitHub ' + githubTotal : '') +
          '（上限 1000）' +
          (showActivity && activity !== 'all' ? ' · ' + activeActivity.label + ' ' + afterActivity.length : '') +
      ...
      return createPortal(createElement('div', {
        ...
        createElement('div', { style: overlay.panel },
          ...
          createElement('div', { style: overlay.bannerDanger, title: RISK_WARNING },
            createElement('strong', { style: overlay.bannerDangerTitle }, '风险提示'),
            RISK_WARNING,
          ),
          ...
          showActivity && createElement('div', { style: overlay.filters },
            ACTIVITY.map(function (w) {
              var count = activityCounts[w.id] || 0
              var active = activity === w.id
              return createElement('button', {
                key: w.id,
                type: 'button',
                style: Object.assign({}, active ? overlay.chipActive : overlay.chip, (w.id !== 'all' && count === 0) ? { opacity: 0.4, cursor: 'default' } : {}),
                disabled: w.id !== 'all' && count === 0,
                onClick: function () { setActivity(w.id) },
              }, w.label + ' ' + count)
            }),
          ),
          ...
          isJobs ? createElement('div', { key: 'jobs-' + safePage + '-' + qn, style: overlay.list },
            ...
            visible.map(function (job) {
              ...
              return createElement('div', { key: job.id, style: overlay.card },
                createElement('div', { style: overlay.cardMain },
                  ...
                  jobDesc ? createElement('div', { style: overlay.cardDesc }, jobDesc) : null,
          ...
          ) : createElement('div', { key: tab + '-' + safePage + '-' + qn + '-' + category + '-' + activity, style: overlay.list },
            ...
            !loading && rows.length === 0 && createElement('div', { style: overlay.empty },
              tab === 'installed'
                ...
                : (category !== 'all' && afterActivity.length
                  ? '该分类下没有匹配的插件。'
                  : (showActivity && activity !== 'all' && searched.length
                    ? '该时间窗口内没有匹配的插件，可改选「含停滞」。'
                    : '没有匹配的插件。'))
            ),
            !(loading && source.length === 0) && visible.map(function (item) {
              ...
              var weeks = weeksSincePush(item, now)
              var pushedLabel = fmtActivityMeta(item, weeks)
              var risk = activityRisk(weeks)
              return createElement('div', { key: key, style: overlay.card },
                createElement('div', { style: overlay.cardMain },
                  createElement('div', { style: overlay.cardTitle },
                    ...
                    createElement('span', {
                      style: overlay.tagDanger,
                      title: '列表内插件均未做木马 / 病毒检查，请自行 clone 并审查后再安装',
                    }, '未审计'),
                    ...
                    risk ? createElement('span', { style: overlay[risk.style], title: risk.title }, risk.label) : null,
                  ),
                  desc ? createElement('div', { style: overlay.cardDesc }, desc) : null,
                  ...
                  createElement('div', { style: overlay.meta },
                    ...
                    pushedLabel
                      ? createElement('span', { title: '仓库最后一次 git push（本地时间）' }, '  ·  ' + pushedLabel)
                      : null,
                ...
                createElement('div', { style: overlay.actions },
                  !isOn && tab !== 'installed' && createElement('button', {
                    ...
                    title: blockRootInstall
                      ? '根仓通常无法加载，请用「安装说明」或复制命令装子目录'
                      : RISK_WARNING,
          ...
          createElement('pre', { style: overlay.log }, log || (RISK_WARNING + '\n安装会进入「安装中」列表；超时会自动重试，也可手动重试。更新、修复、卸载后需要重启 DSH 才会生效。')),
        ),
      ), document.body)
    }
    ...
    function FooterButton(props) {
      ...
      var onAction = function (kind, item) {
        if (kind === 'install') {
          var name = item.id || item.packageName || item.name || item.spec || '该插件'
          if (!window.confirm(
            '风险提示：' + RISK_WARNING + '\n\n一键安装不会替你做安全检查。确定仍要安装 ' + name + ' 吗？'
          )) return
    ...
    var overlay = {
      ...
      filters: { display: 'flex', gap: 6, padding: '0 20px 8px', flexWrap: 'wrap' },
      chip: {
