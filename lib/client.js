window.__ModuleLoader__.load({
  ...
  factory: (require) => {
    ...
    function Overlay(props) {
      ...
      var isJobs = tab === 'jobs'
      var source = tab === 'installed' ? installed : tab === 'jobs' ? jobs : catalog
      var sourceKey = (source[0] && (source[0].id || source[0].name || source[0].spec)) || ''
      var rows = !qn ? source : source.filter(function (item) {
        return ((item.id || '') + ' ' + (item.name || '') + ' ' + (item.title || '') + ' ' + (item.description || '') + ' ' + (item.spec || '') + ' ' + (item.error || '') + ' ' + (item.log || '')).toLowerCase().includes(qn)
      })
      ...
      var needRestart = jobs.some(function (job) { return job.status === 'success' && job.needRestart })

      useEffect(function () {
        setPage(1)
      }, [tab, qn, source.length, sourceKey])
        ...
        createElement('div', { style: overlay.panel },
          ...
          createElement('div', { style: overlay.toolbar },
            createElement('input', {
              style: overlay.input,
              value: query,
              placeholder: isJobs ? '搜索安装任务' : '搜索仓库名或描述（本地过滤）',
          ...
          isJobs && needRestart && createElement('div', { style: overlay.banner },
            '有插件已写入 profile，请用启动器重启 DSH 后生效。',
          ),
          isJobs && jobsFailed > 0 && createElement('div', { style: overlay.bannerWarn },
              ...
              return createElement('div', { key: key, style: overlay.card },
                ...
                createElement('div', { style: overlay.actions },
                  !isOn && tab !== 'installed' && createElement('button', {
                    type: 'button', style: overlay.btn, disabled: busy || inQueue,
      ...
      var loadJobs = function () {
        ...
        return json('/jobs')
          .then(function (d) {
            ...
            setJobs(list)
            setNow(Date.now())
            var becameDone = list.some(function (job) {
            ...
            if (becameDone) {
              var plainJob = list.find(function (job) {
                if (!(job.plainDependency && job.status === 'success')) return false
                var before = prev.find(function (row) { return row.id === job.id })
                return !before || before.status === 'installing'
              })
              if (plainJob) {
                setLog('安装完成，但装成了普通依赖（未入 bundles）：' + (plainJob.title || plainJob.spec)
